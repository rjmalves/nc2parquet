...
impl fmt::Display for PostProcessError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            PostProcessError::ColumnNotFound(col) => write!(f, "Column '{}' not found in DataFrame", col),
...
impl ProcessingPipeline {
    ...
    pub fn from_config(config: &ProcessingPipelineConfig) -> PostProcessResult<Self> {
        let mut pipeline = Self {
            name: config.name.clone().unwrap_or_else(|| "Configured Pipeline".to_string()),
            processors: Vec::new(),
        };
    ...
    pub fn execute(&mut self, mut df: DataFrame) -> PostProcessResult<DataFrame> {
        debug!("Executing pipeline '{}' with {} processors", self.name, self.processors.len());
        
        if self.processors.is_empty() {
            debug!("Pipeline '{}' is empty, returning DataFrame unchanged", self.name);
            return Ok(df);
        }
        ...
        for (i, processor) in self.processors.iter().enumerate() {
            ...
            debug!("Executing processor {} '{}' - input shape: {:?}", 
                   i + 1, processor_name, df.shape());
            
            df = processor.process(df)?;
            
            debug!("Processor '{}' completed - output shape: {:?}", 
                   processor_name, df.shape());
        }
    ...
    match config {
        ...
        ProcessorConfig::DatetimeConvert { column, base, unit } => {
            let base_dt = DateTime::parse_from_rfc3339(base)
                .map_err(|e| PostProcessError::ConfigurationError(
                    format!("Invalid base datetime '{}': {}", base, e)
                ))?
                .with_timezone(&Utc);
            Ok(Box::new(DateTimeConverter::new(column.clone(), base_dt, unit.clone())))
        }
        ProcessorConfig::UnitConvert { column, from_unit, to_unit } => {
            Ok(Box::new(UnitConverter::new(column.clone(), from_unit.clone(), to_unit.clone())))
        }
        ProcessorConfig::Aggregate { group_by, aggregations } => {
            Ok(Box::new(Aggregator::new(group_by.clone(), aggregations.clone())))
        }
        ProcessorConfig::ApplyFormula { target_column, formula, source_columns } => {
...
impl DateTimeConverter {
    pub fn new(column: String, base_datetime: DateTime<Utc>, unit: TimeUnit) -> Self {
        Self { column, base_datetime, unit }
    }
}
...
impl UnitConverter {
    pub fn new(column: String, from_unit: String, to_unit: String) -> Self {
        ...
        Self { column, from_unit, to_unit, conversion_factor }
    }
    
    pub fn with_conversion_factor(column: String, from_unit: String, to_unit: String, factor: f64) -> Self {
        Self { column, from_unit, to_unit, conversion_factor: factor }
    }
    ...
    fn process(&self, mut df: DataFrame) -> PostProcessResult<DataFrame> {
        ...
        for (old_name, new_name) in &self.mappings {
            ...
            if !column_names.contains(&old_name.as_str()) {
                warn!("Column '{}' not found in DataFrame, skipping rename", old_name);
                continue;
            }
...
impl PostProcessor for DateTimeConverter {
    fn process(&self, df: DataFrame) -> PostProcessResult<DataFrame> {
        debug!("Converting column '{}' to datetime using base {} and unit {:?}", 
               self.column, self.base_datetime.to_rfc3339(), self.unit);
    ...
    fn output_schema(&self, input_schema: &Schema) -> PostProcessResult<Schema> {
        ...
        new_schema.with_column(
            format!("{}_datetime", self.column).into(), 
            DataType::String
        );
...
impl PostProcessor for UnitConverter {
    fn process(&self, df: DataFrame) -> PostProcessResult<DataFrame> {
        debug!("Converting column '{}' from {} to {} (factor: {})", 
        ...
        let result = if (self.from_unit.to_lowercase() == "kelvin" || self.from_unit.to_lowercase() == "k") 
                      && (self.to_unit.to_lowercase() == "celsius" || self.to_unit.to_lowercase() == "c") {
            // Special case: Kelvin to Celsius (K - 273.15)
            df.lazy()
                .with_columns([
                    (col(&self.column) - lit(273.15)).alias(&self.column)
                ])
                .collect()?
        } else if (self.from_unit.to_lowercase() == "celsius" || self.from_unit.to_lowercase() == "c")
            ...
            df.lazy()
                ...
                .collect()?
        } else if (self.from_unit.to_lowercase() == "fahrenheit" || self.from_unit.to_lowercase() == "f")
                 && (self.to_unit.to_lowercase() == "celsius" || self.to_unit.to_lowercase() == "c") {
...
impl PostProcessor for Aggregator {
    fn process(&self, df: DataFrame) -> PostProcessResult<DataFrame> {
        debug!("Aggregating data with group_by: {:?}, aggregations: {:?}", 
        ...
        for (col_name, agg_op) in &self.aggregations {
            let expr = match agg_op {
                AggregationOp::Mean => col(col_name).mean(),
                AggregationOp::Sum => col(col_name).sum(),
                AggregationOp::Min => col(col_name).min(),
                AggregationOp::Max => col(col_name).max(),
                AggregationOp::Count => col(col_name).count(),
                AggregationOp::Std => col(col_name).std(1), // Use population std
                AggregationOp::Var => col(col_name).var(1), // Use population var
                AggregationOp::First => col(col_name).first(),
                AggregationOp::Last => col(col_name).last(),
            };
            agg_exprs.push(expr.alias(&format!("{}_{:?}", col_name, agg_op).to_lowercase()));
        }
...
impl PostProcessor for FormulaApplier {
    fn process(&self, df: DataFrame) -> PostProcessResult<DataFrame> {
        debug!("Applying formula '{}' to create column '{}'", 
...
impl FormulaApplier {
    pub fn new(target_column: String, formula: String, source_columns: Vec<String>) -> Self {
        Self { target_column, formula, source_columns }
    }

    /// Apply the formula to create the target column
    fn apply_formula(&self, df: DataFrame) -> PostProcessResult<DataFrame> {
        // Simple formula parser - supports basic arithmetic operations
        let formula = self.formula.trim();
        
        // Handle common formulas
        let result = if formula.contains('+') || formula.contains('-') || formula.contains('*') || formula.contains('/') {
            ...
            if self.source_columns.contains(&formula.to_string()) {
                df.lazy()
                    .with_columns([col(formula).alias(&self.target_column)])
                    .collect()?
            } else {
                // Try to parse as constant
                match formula.parse::<f64>() {
                    Ok(constant) => df.lazy()
                        .with_columns([lit(constant).alias(&self.target_column)])
                        .collect()?,
                    Err(_) => return Err(PostProcessError::ProcessingError(
                        format!("Unable to parse formula: {}", formula)
                    ))
                }
            }
        };
    ...
    /// Parse arithmetic formulas like "a + b", "a * 2", etc.
    fn parse_arithmetic_formula(&self, df: DataFrame, formula: &str) -> PostProcessResult<DataFrame> {
        // Very basic parser - handles simple binary operations
        let operators = [" + ", " - ", " * ", " / "];
        ...
        Err(PostProcessError::ProcessingError(
            format!("Unable to parse arithmetic formula: {}", formula)
        ))
    }
    ...
    fn parse_function_formula(&self, df: DataFrame, formula: &str) -> PostProcessResult<DataFrame> {
        if formula.starts_with("sqrt(") && formula.ends_with(")") {
            ...
            let operand = self.parse_operand(inner)?;
            ...
            Err(PostProcessError::ProcessingError(
                format!("Unsupported function in formula: {}", formula)
            ))
        }
    }
    
    /// Parse an operand (column name or constant)
    fn parse_operand(&self, operand: &str) -> PostProcessResult<Expr> {
        let operand = operand.trim();
        
        // Check if it's a column name
        if self.source_columns.contains(&operand.to_string()) {
            Ok(col(operand))
        } else {
            // Try to parse as constant
            match operand.parse::<f64>() {
                Ok(constant) => Ok(lit(constant)),
                Err(_) => Err(PostProcessError::ProcessingError(
